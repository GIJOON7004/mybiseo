...
const i18n: Record<Lang, Record<string, string>> = {
  ko: {
    ...
    ctaWeb: `온라인 상담  ${APP_DOMAIN}`,
    disclaimer: `본 보고서는 ${APP_DOMAIN}의 AI 가시성 진단 시스템에 의해 생성되었습니다. AI 플랫폼의 내부 알고리즘은 비공개이므로, 진단 결과는 전략 수립을 위한 참고 자료입니다.`,
    statusPass: "통과",
    statusWarning: "주의",
    statusFail: "실패",
    naverLabel: "네이버",
    ...
    aiSimDisclaimer: "※ AI 시뮬레이터 결과는 진단 시점의 추정값이며, AI 모델 업데이트·사용자 위치·쿼리 맥락에 따라 실제 결과와 다를 수 있습니다.",
  ...
  en: {
    ...
    ctaWeb: `Online  ${APP_DOMAIN}`,
    disclaimer: `This report was generated by the AI Visibility Diagnostic System at ${APP_DOMAIN}. Diagnostic results serve as reference material for strategy development.`,
    statusPass: "Pass",
    ...
    aiSimDisclaimer: "※ AI simulator results are estimates at the time of diagnosis and may differ from actual results depending on AI model updates, user location, and query context.",
  ...
  th: {
    ...
    ctaWeb: `ออนไลน์  ${APP_DOMAIN}`,
    disclaimer: `รายงานนี้สร้างโดยระบบวินิจฉัย AI Visibility ของ ${APP_DOMAIN}`,
    statusPass: "ผ่าน",
    statusWarning: "เตือน",
    statusFail: "ไม่ผ่าน",
    naverLabel: "Naver",
    ...
    aiSimDisclaimer: "※ ผลตัวจำลอง AI เป็นค่าประมาณ ณ เวลาวินิจฉัย ผลจริงอาจแตกต่างกันขึ้นอยู่กับการอัปเดตโมเดล AI ตำแหน่งผู้ใช้ และบริบทของคำค้นหา",
...
.card-pass-top { border-top: 3px solid var(--pass); }
.info-box-pass { border-left: 4px solid var(--pass); background: #f0fdf4; }
...
function pageFooter(pageNum: number, totalPages: number, brand: string): string {
  return `<div class="page-footer">
    ...
    <span style="color:var(--gray-3);font-size:9pt;">CONFIDENTIAL | ${APP_DOMAIN}</span>
  </div>`;
}
...
function buildCoverPage(t: Record<string, string>, hospitalName: string, displayUrl: string, score: number, grade: string, summary: { passed: number; warnings: number; failed: number }, lang: Lang): string {
  ...
  return `<div class="page cover">
    ...
    <!-- 우하단 ${APP_DOMAIN} -->
    <div style="position:absolute;bottom:14px;right:40px;font-size:8pt;color:rgba(255,255,255,0.4);z-index:2;">${APP_DOMAIN}</div>
  </div>`;
}
...
function buildExecutiveSummaryPage(t: Record<string, string>, url: string, score: number, grade: string, summary: { passed: number; warnings: number; failed: number }, rd: RealityDiagnosis | null, categories: SeoAuditResult["categories"], lang: Lang, pageNum: number, totalPages: number): string {
  ...
  // H-4+H-5: A등급 이상(80점+)은 "추가 성장 기회" 프레이밍, 미만은 "매출 손실" 프레이밍
  const isHighScore = score >= 80;
  const accentColor = isHighScore ? 'var(--pass)' : 'var(--danger)';
  const cardTopClass = isHighScore ? 'card-pass-top' : 'card-danger-top';
  const infoBoxClass = isHighScore ? 'info-box-pass' : 'info-box-danger';
  const summaryTitle = lang === 'ko' ? '핵심 요약' : lang === 'en' ? 'Executive Summary' : 'สรุปผล';
  const summarySubtitle = lang === 'ko'
    ? (isHighScore ? '이 병원의 추가 성장 기회를 분석합니다' : '지금 이 병원에 무슨 일이 일어나고 있나')
    : lang === 'en'
    ? (isHighScore ? 'Additional growth opportunities for this hospital' : 'What is happening to this hospital right now')
    : (isHighScore ? 'โอกาสเติบโตเพิ่มเติมของโรงพยาบาลนี้' : 'สิ่งที่เกิดขึ้นกับโรงพยาบาลนี้ตอนนี้');
  ...
  return `<div class="page">
    ...
    <div class="content">
      ...
      <!-- 핵심 요약 인포박스 (H-4+H-5: 점수 기반 프레이밍) -->
      <div class="info-box ${infoBoxClass}" style="border-left-width:6px;padding:14px 18px;margin-top:14px;">
        <div style="font-size:13pt;font-weight:600;color:var(--text);line-height:1.7;">
          ${isHighScore
            ? `${esc(hospitalName)}${lang === 'ko' ? '은 현재 우수한 성과를 보이고 있으며, 매월 약 ' : ' is performing well, with approximately '}<span style="color:${accentColor};font-size:16pt;font-weight:700;">${missedPatients}${lang === 'ko' ? '명' : ''}</span>${lang === 'ko' ? '의 추가 유입 가능 트래픽과 ' : ' additional capturable visitors and '}<span style="color:${accentColor};font-size:16pt;font-weight:700;">${esc(revenueLoss)}</span>${lang === 'ko' ? '의 추가 성장 기회가 있습니다' : ' in additional growth opportunity every month'}`
            : `${esc(hospitalName)}${lang === 'ko' ? '은 현재 매월 약 ' : ' is currently missing approximately '}<span style="color:${accentColor};font-size:16pt;font-weight:700;">${missedPatients}${lang === 'ko' ? '명' : ''}</span>${lang === 'ko' ? '의 미유입 잠재 환자와 ' : ' potential patients and '}<span style="color:${accentColor};font-size:16pt;font-weight:700;">${esc(revenueLoss)}</span>${lang === 'ko' ? '의 잠재 매출 기회가 존재합니다' : ' in potential revenue opportunity every month'}`
          }
        </div>
      </div>
      ...
      <div class="grid-3" style="margin-top:16px;">
        <div class="card ${cardTopClass}" style="text-align:center;padding:18px 14px;">
          <div style="margin-bottom:8px;"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="${accentColor}" stroke-width="2"><path d="M12 19V5M5 12l7-7 7 7"/></svg></div>
          <div style="font-size:32pt;font-weight:700;color:${accentColor};line-height:1;">${esc(revenueLoss)}</div>
          <div style="font-size:10px;font-weight:500;color:var(--text);margin-top:6px;">${lang === 'ko' ? (isHighScore ? '추가 성장 기회 금액' : '예상 잠재 매출 기회') : 'Est. Revenue Opportunity'}</div>
          <div style="font-size:9px;color:var(--gray-3);margin-top:4px;">${lang === 'ko' ? (isHighScore ? `추가 유입 가능 트래픽 ${missedPatients}명 기준` : `미유입 잠재 환자 ${missedPatients}명 기준`) : `Based on ${missedPatients} potential visitors`}</div>
        </div>
        <div class="card ${cardTopClass}" style="text-align:center;padding:18px 14px;">
